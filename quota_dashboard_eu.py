...
def attach_changes(rows, history):
    """Attach change vs the most recent prior day's snapshot to each row."""
    today = date.today().isoformat()
    prior = sorted(d for d in history.keys() if d < today)
    ref = prior[-1] if prior else None
    prev = history.get(ref) if ref else None
    for r in rows:
        r["change"] = None
        r["change_ref"] = ref
        if prev is not None and r["balance"] is not None:
            pb = prev.get(r["order"])
            if pb is not None:
                r["change"] = round(r["balance"] - float(pb), 1)
    return ref
...
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>EU Quota Daily Movers</title>
<style>
...
.navlink{display:inline-block;margin:14px 0 4px;padding:9px 16px;background:var(--ink);color:#fff;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600;}
.summary{display:flex;gap:12px;margin:18px 0 8px;flex-wrap:wrap;}
...
<body><div class="wrap">
<header><div><h1>EU Steel Quota &mdash; Daily Movers</h1>
<div class="sub">%%RANGE%%</div></div>
<div class="sub">Refreshed<br><strong>%%TS%%</strong></div></header>
<a class="navlink" href="index.html">&larr; Dashboard</a>
<a class="navlink" href="trends.html">Historical trends &rarr;</a>
<div class="summary">
  <div class="stat"><div class="n">%%NMOVED%%</div><div class="l">Quotas that moved</div></div>
  <div class="stat crit"><div class="n">%%TOTDRAWN%%</div><div class="l">Total drawn (t)</div></div>
  <div class="stat crit"><div class="n">%%BIGGEST%%</div><div class="l">Biggest single drop (t)</div></div>
</div>
%%BODY%%
<footer>Compares the latest balances against the previous day's snapshot. Drops = quota consumed (imports cleared); gains = returns/adjustments.<br>
Tracking begins from the first snapshot, so day-over-day figures build up over time. Source: EU tariff quota (QUOTA) database.</footer>
</div></body></html>"""


def build_movers(rows, ref):
    now = datetime.now(UK_TZ) if UK_TZ else datetime.now()
    ts = now.strftime("%A %d %B %Y, %H:%M") + (" UK time" if UK_TZ else " UTC")
    moved = [r for r in rows if r.get("change") not in (None, 0)]
    drops = sorted([r for r in moved if r["change"] < 0], key=lambda r: r["change"])
    gains = sorted([r for r in moved if r["change"] > 0], key=lambda r: -r["change"])
    total_drawn = -sum(r["change"] for r in drops) if drops else 0
    ...
    if ref is None:
        body = ("<p class='none'>First snapshot recorded today &mdash; daily movements will "
                "appear here from the next update onwards.</p>")
        rng = "Waiting for a second snapshot to compare against"
        nmoved = tot = big = "-"
    else:
        rng = "Changes since " + ref
        nmoved = str(len(moved))
        tot = fmt(total_drawn, 0)
        big = fmt(-drops[0]["change"], 0) if drops else "0"
        parts = []
        if drops:
        ...
        if not parts:
            parts.append("<p class='none'>No quota movements since the last snapshot.</p>")
        body = "".join(parts)

    out = MOVERS_PAGE
    for tok, val in (("%%TS%%", ts), ("%%RANGE%%", rng), ("%%NMOVED%%", nmoved),
                     ("%%TOTDRAWN%%", tot), ("%%BIGGEST%%", big), ("%%BODY%%", body)):
        out = out.replace(tok, val)
    return out
